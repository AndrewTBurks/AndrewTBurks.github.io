...
**<code style="font-size: 1.2rem;">SAGE2.SnippetExternalOp</code>**
...
![Inspecting the Data Graph](../images/528p2/HoverNode.PNG "Hover over a node in the graph to view the operation used at that node.")
...
![Viewing the Line Chart of the Selected Point](../images/528p2/SelectVegaLiteLineChart.PNG "Open the line chart to view time-series data for the selected data point.")
...
![Change the Scale between new Attributes](../images/528p2/ColorAndScalePointsByState.PNG "The 3D Encodings menu dynamically updates to display new options for this data which has different attributes.")
...
- there is not enough two-way dynamic linkage between the backend and the VR application
  - the current selection isn't shown, and the image doesn't update automatically on selection

The implemented client-server architecture is functional and the analysis is transferrable between platforms. With the foundation in place, the Unity application can be extended further to support more complex interaction, either within the application or between the application and the analysis.
